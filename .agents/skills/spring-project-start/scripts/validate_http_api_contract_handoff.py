...
def main()->int:
 ...
 try:
  root=args.target.resolve(strict=True); handoff_path=argument_path(root,args.handoff,"handoff"); view_path=argument_path(root,args.view,"handoff view"); saved=load_object(handoff_path)
  if saved.get("httpApiContractHandoffVersion")!=1 or view_path!=handoff_path.with_suffix(".md") or not view_path.is_file(): raise ValueError("handoff identity or view is invalid")
  inputs=saved["inputs"]; expected=build(root,argument_path(root,root/inputs["applicationReceipt"]["path"],"application receipt"),argument_path(root,root/inputs["approvedRoute"]["path"],"approved route"),argument_path(root,root/inputs["feature"]["path"],"feature"),argument_path(root,root/inputs["projectBrief"]["path"],"project brief"),argument_path(root,root/inputs["technologyProfile"]["path"],"technology profile"),saved["contractId"],[handoff_path,view_path])
