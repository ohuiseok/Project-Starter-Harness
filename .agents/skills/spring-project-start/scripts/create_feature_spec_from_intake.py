...
def main()->int:
 ...
 try:
  ...
  receipt={"continuationConsumptionVersion":2,"intake":reference(intake_path,root),"draft":{"path":draft_path.relative_to(root).as_posix(),"sha256":hashlib.sha256(draft_bytes).hexdigest()},"view":{"path":view_path.relative_to(root).as_posix(),"sha256":hashlib.sha256(view_bytes).hexdigest()},"baseFeature":reference(existing_path,root) if existing else None,"featureId":spec["feature"]["id"],"state":"AWAITING_USER_DECISIONS"}; receipt_bytes=(json.dumps(receipt,ensure_ascii=False,indent=2)+"\n").encode()
