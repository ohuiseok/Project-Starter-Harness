...
def operation_scope(document: dict, selected_ids: set[str]) -> dict:
    ...
    for path, method, operation in operations(document):
        ...
        path_parameters = document.get("paths", {}).get(path, {}).get("parameters", []); effective_security = operation.get("security", document.get("security", []))
        operation_documents[operation_id] = {"method": method.upper(), "path": path, "operation": operation, "pathParameters": path_parameters, "effectiveSecurity": effective_security}
...
def build(root: Path, application_path: Path, route_path: Path, feature_path: Path, project_path: Path, profile_path: Path, contract_id: str, handoff_paths: list[Path] | None = None) -> dict:
    ...
    baseline = None; scope = None; discovery_refs = [item for item in route["inputs"]["codeEvidence"] if item["kind"].startswith(f"HTTP_API_DISCOVERY:{contract_id}:")]
    ...
    if selected["disposition"] in {"EXTEND", "REUSE"}:
        ...
        document = load_object(baseline_path); selected_ids = {item.get("operationId") for item in candidate["requirementMatches"] if isinstance(item.get("operationId"), str)}
    ...
    return {"httpApiContractHandoffVersion": 1, "status": "READY" if not semantic_blockers else "BLOCKED", "contractId": contract_id, "disposition": selected["disposition"], "target": selected["target"], "inputs": {"applicationReceipt": reference(application_path, root), "approvedRoute": reference(route_path, root), "feature": reference(feature_path, root), "projectBrief": reference(project_path, root), "technologyProfile": reference(profile_path, root), "discovery": reference(discovery_path, root)}, "baseline": baseline, "operationScope": scope, "expectedOutputs": outputs, "blockers": semantic_blockers, "effects": {"adapterExecuted": False, "contractChanged": False, "sourceChanged": False, "existingOpenApiChanged": False, "gitCommitOrPush": "NOT_RUN"}}
...
def main() -> int:
    ...
    parser.add_argument("--contract-id", required=True); args = parser.parse_args(); written=[]
    try:
        ...
        value=build(root,paths["application"],paths["route"],paths["feature"],paths["project-brief"],paths["profile"],args.contract_id,[paths["output"],paths["view"]])
