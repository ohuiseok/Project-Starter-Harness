...
def build_completion(root:Path,transaction_id:str,project_path:Path,feature_path:Path,dry_path:Path,verification_path:Path,completed_at:str)->dict:
    ...
    record_path=root/MANAGED/"implementation-transactions"/transaction_id/"transaction.json"; record=load_object(record_path)
    ...
    return {"milestoneCompletionVersion":1,"featureId":feature["feature"]["id"],"featureName":feature["feature"]["name"],"userValue":feature["feature"]["userValue"],"state":"APPLIED_AND_VERIFIED","completedAt":completed_at,"projectBrief":ref(project_path,root),"featureSpec":ref(feature_path,root),"implementationPlan":dry["implementationPlan"],"dryRun":ref(dry_path,root),"verification":ref(verification_path,root),"transaction":ref(record_path,root),"baseline":ref(baseline_path,root),"requirements":requirements,"summary":{"requirementsPassed":len(requirements),"appliedFiles":len(manifest["files"]),"automatedTests":len({path for item in requirements for path in item["testPaths"]})},"postApplyRuntimeVerification":"NOT_RUN","gitCommitOrPush":"NOT_RUN"}

def validate_progress(value:dict,root:Path)->None:
    required={"progressVersion","target","project","completedMilestones","current","nextCandidates","unknowns","updatedAt"}
    ...
    for item in value["completedMilestones"]:
        if not isinstance(item,dict) or set(item)!={"featureId","name","userValue","state","completedAt","completionReport"} or item["state"]!="APPLIED_AND_VERIFIED" or not isinstance(item["userValue"],str) or not item["userValue"].strip(): raise ValueError("completed milestone entry is invalid")
        report=root/item["completionReport"]["path"]
        if not report.is_file() or sha(report)!=item["completionReport"]["sha256"]: raise ValueError("completion report evidence changed")
...
def next_candidates(project:dict,completed:set[str])->list[dict]:
    result=[]
    for item in sorted(project["featureCandidates"],key=lambda value:value["recommendedOrder"]):
        if item["id"] in completed or item["status"]=="DEFERRED" or item["blockingUnknownIds"] or any(dep not in completed for dep in item["dependsOn"]): continue
        result.append({"featureId":item["id"],"name":item["name"],"userValue":item["userValue"],"reason":item["recommendationReason"]})
    return result
...
def build_progress(existing:dict|None,project:dict,completion:dict,completion_ref:dict,root:Path)->dict:
    ...
    candidates=next_candidates(project,{item["featureId"] for item in milestones}); unknowns=[{"id":item["id"],"question":item["question"],"blocking":item["blocking"]} for item in project["unknowns"] if item["status"]!="RESOLVED"]
    return {"progressVersion":1,"target":str(root),"project":{"name":project["project"]["name"],"goal":project["project"]["goal"]},"completedMilestones":milestones,"current":{"state":"READY_FOR_NEXT_FEATURE" if candidates else "NO_ELIGIBLE_FEATURE","recommendedFeatureId":candidates[0]["featureId"] if candidates else None},"nextCandidates":candidates,"unknowns":unknowns,"updatedAt":completion["completedAt"]}
