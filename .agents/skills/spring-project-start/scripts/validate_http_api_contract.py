#!/usr/bin/env python3
"""Validate a CREATE HTTP API contract and its OpenAPI artifact."""
...
def main() -> int:
    ...
    try:
        approved, blockers, _ = validate_http_contract(
        ...
        print(f"HTTP_API_CONTRACT_VALID: no\nERROR: {error}")
        return 1
    print("HTTP_API_CONTRACT_VALID: yes")
    print(f"APPROVED: {'yes' if approved else 'no'}")
    print(f"HTTP_API_READY: {'yes' if ready else 'no'}")
