...
from run_relational_migration_verification import sha, version_key
...
def validate(report: dict, report_path: Path, target: Path) -> None:
    ...
    if not isinstance(report, dict) or set(report) != required or report["migrationVerificationReportVersion"] != 3: raise ValueError("migration verification report is invalid")
    ...
    result_keys = {"state", "events", "cleanup", "images", "appliedMigrations", "targetDatabaseAccessed", "targetSourceFilesChanged", "persistentVolumeCreated"}
...
def render(report: dict) -> str:
    ...
    lines.extend(["", "## 해석", "", "- PASSED는 선택한 빈 격리 PostgreSQL에서 전체 versioned migration chain의 migrate/validate와 Flyway info 이력 대조가 성공했다는 의미", "- production 데이터·권한·성능·lock·rollback 성공을 의미하지 않음", ""]); return "\n".join(lines)
