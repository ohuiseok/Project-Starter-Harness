...
def validate(report: dict, report_path: Path, target: Path) -> None:
    ...
    if not isinstance(report, dict) or set(report) != required or report["migrationVerificationReportVersion"] != 2: raise ValueError("migration verification report is invalid")
    ...
    result_keys = {"state", "events", "cleanup", "images", "targetDatabaseAccessed", "targetSourceFilesChanged", "persistentVolumeCreated"}
