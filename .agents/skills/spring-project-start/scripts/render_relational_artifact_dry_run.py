...
def artifact_plan(plan: dict, physical: dict, contract_path: Path, model_path: Path, profile_path: Path, profile: dict, target: Path) -> None:
    keys = {"artifactPlanVersion", "planId", "physicalContractSha256", "physicalModelSha256", "profileSha256", "databaseName", "credentialBindings", "testcontainers"}
    if not isinstance(plan, dict) or set(plan) != keys or plan["artifactPlanVersion"] != 1: raise ValueError("relational artifact plan is invalid")
    ...
    if not isinstance(bindings, dict) or set(bindings) != {"username", "password"}: raise ValueError("credentialBindings must identify username and password")
    names = physical["provisioningPlan"]["compose"]["secretNames"] if physical["provisioningPlan"]["compose"] else []
    if any(not isinstance(value, str) or not SECRET.fullmatch(value) for value in bindings.values()): raise ValueError("credential bindings must be secret names")
    if bindings["username"] == bindings["password"] or set(bindings.values()) != set(names): raise ValueError("credential bindings must map the approved secret names exactly")
...
def compose(physical: dict, plan: dict) -> str:
    item = physical["provisioningPlan"]["compose"]; bindings = plan["credentialBindings"]
...
def testcontainers_java(physical: dict, plan: dict) -> str:
    test = plan["testcontainers"]; bindings = plan["credentialBindings"]; image = physical["provisioningPlan"]["testcontainers"]["imageReference"]
    return "\n".join([f"package {test['packageName']};", "", "import org.springframework.boot.test.context.TestConfiguration;", "import org.springframework.boot.testcontainers.service.connection.ServiceConnection;", "import org.springframework.context.annotation.Bean;", "import org.testcontainers.containers.PostgreSQLContainer;", "", "@TestConfiguration(proxyBeanMethods = false)", f"public class {test['className']} {{", "    @Bean", "    @ServiceConnection", "    PostgreSQLContainer<?> postgresContainer() {", f"        return new PostgreSQLContainer<>(\"{image}\")", f"                .withDatabaseName(\"{plan['databaseName']}\")", f"                .withUsername(requiredEnvironment(\"{bindings['username']}\"))", f"                .withPassword(requiredEnvironment(\"{bindings['password']}\"));", "    }", "", "    private static String requiredEnvironment(String name) {", "        String value = System.getenv(name);", "        if (value == null || value.isBlank()) {", "            throw new IllegalStateException(\"Required test credential environment variable is missing: \" + name);", "        }", "        return value;", "    }", "}", ""])
...
def main() -> int:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="relational-artifact-dry-run-") as temporary:
            ...
            artifacts = [("FLYWAY_SQL", physical["migrationPlan"]["plannedSourcePath"], sql(physical))]
            ...
            baseline, modes = load_baseline(args.baseline_manifest); changes = compare(rendered, root, baseline, modes)
        ...
        report = {"relationalArtifactDryRunVersion": 1, "physicalContract": {"path": str(args.physical_contract), "sha256": sha(args.physical_contract)}, "physicalModelSha256": sha(args.physical_model), "artifactPlan": {"path": str(args.artifact_plan), "sha256": sha(args.artifact_plan)}, "target": str(root), "generatedArtifacts": previews, "plannedChanges": changes, "recoveryAssessment": {"required": physical["migrationPlan"]["requiredRecovery"], "renderedDdlClass": "TRANSACTIONAL_CREATE_ONLY", "isolatedDatabaseVerified": False}, "targetSourceChanged": False, "databaseOrContainerChanged": False, "readyForApproval": changes["state"] == "COMPUTED", "executionReady": False}
