...
from spring_code_verification_v2 import cache,git_state,runtime_mounts
...
def render_plan(plan:dict,result:dict)->str:
 cmd=plan["command"];shown=" ".join([cmd["executable"],*cmd["arguments"]]);mounts=plan["effects"].get("runtimeMounts");mount_summary=[] if mounts is None else [f"- Java 전용 runtime 설정: {mounts['state']} · {len(mounts['mounts'])}개 디렉터리 · {mounts['fileCount']}개 파일"];lines=["# 적용 후 실제 검증","","## 결론","",f"- 실행 준비: {'예' if plan['readyForApproval'] else '아니요'}","- 적용 상태: `APPLIED_PREVERIFIED`",f"- 명령: `{shown}`","- 실행 위치: 프로젝트의 격리된 임시 복사본","- 실제 target 소스·설정·baseline: 변경하지 않음","- 네트워크·Docker·DB·포트: 사용하지 않음",*mount_summary,f"- 제한 시간: {plan['limits']['timeoutSeconds']}초",f"- 로그 최대 크기: {plan['limits']['maxLogBytes']} bytes","- 결과가 통과해도 milestone 완료·progress 변경은 별도 승인","","## 실패 시 선택","","- 테스트 수정안 만들기 / 동일 검증 재실행 / 보류","- 환경 UNKNOWN 해결 / 제한 시간 조정 / 다른 검증 요청","- 기타 / 자연어 입력",""]
