...
def render(plan: dict, plan_hash: str) -> str:
    return "\n".join([f"# {plan['planId']} migration 격리 검증 계획", "", f"<!-- verification-plan-sha256: {plan_hash} -->", "", "## 검증 대상", "", f"- migration: `{plan['migration']['path']}`", f"- PostgreSQL: `{plan['images']['postgres']}`", f"- Flyway: `{plan['images']['flyway']}`", f"- 임시 DB/schema: `{plan['database']['name']}` / `{plan['database']['schema']}`", "", "## 실제로 발생하는 로컬 효과", "", "- Docker 이미지가 없으면 내려받음", "- 외부 통신이 차단된 임시 Docker network 생성", "- 포트를 공개하지 않는 PostgreSQL 컨테이너 생성", f"- 최대 {plan['limits']['tmpfsBytes']} bytes tmpfs 사용", "- Flyway migrate와 validate 프로세스 실행", "- 검증 evidence 보고서 파일 생성", "- 완료 또는 실패 후 컨테이너와 network 제거", "", "## 발생하지 않는 일", "", "- 프로젝트의 실제 DB 또는 production DB 접속 안 함", "- 프로젝트 Compose 서비스 시작 안 함", "- persistent Docker volume 생성 안 함", "- 대상 migration이나 소스 파일 변경 안 함", "- 애플리케이션 credential 사용·저장 안 함", "", "## 한계", "", "- host가 swap을 사용하면 tmpfs 메모리 페이지가 swap에 기록될 가능성이 있음", "- production 권한·데이터·성능·lock 시간·rollback은 검증하지 않음", "- 성공은 선택한 이미지 조합의 빈 격리 DB에서 migrate/validate가 통과했다는 의미", "", "## 다음 행동", "", "- 추천: 이 정확한 계획을 별도 승인하고 격리 검증 실행", "- 이미지 또는 제한값 수정", "- 실행하지 않고 보류", ""])
