...
def render(plan: dict, plan_hash: str) -> str:
    lines = [f"# {plan['planId']} migration 격리 검증 계획", "", f"<!-- verification-plan-sha256: {plan_hash} -->", "", "## 한눈에 보기", "", f"- 확인: 승인된 versioned migration {len(plan['migrations'])}개가 빈 격리 PostgreSQL에서 순서대로 적용되는지", "- 확인하지 않음: production 데이터·권한·성능·rollback", "- 실행 흐름: 환경 확인 → 이미지 준비 → DB 시작 → migrate → validate → info 이력 대조 → 정리", f"- 최대 대기: DB {plan['limits']['startupTimeoutSeconds']}초, 명령별 {plan['limits']['commandTimeoutSeconds']}초", "", "## 승인할 migration chain", ""]
