...
def main()->int:
 ...
 try:
  ...
  with apply_lock(root):
   ...
   plan=load_object(plan_path);validate_plan(plan,plan_path,root);validate_approval(root,approval_path,plan_path);snapshot=manifest(root);paths=set(snapshot["files"]);consumed=root/MANAGED/"approvals"/"post-apply-v2"/(plan["attemptId"]+".json")
   if consumed.exists() or consumed.is_symlink():raise ValueError("this verification attempt approval was already consumed")
   consumed.parent.mkdir(parents=True,exist_ok=True);atomic_create((json.dumps({"attemptId":plan["attemptId"],"plan":reference(plan_path,root),"approval":reference(approval_path,root),"consumedAt":dt.datetime.now(dt.timezone.utc).isoformat()},ensure_ascii=False,indent=2)+"\n").encode(),consumed)
   ...
   log_dir=root/MANAGED/"logs"/"post-apply-v2";log_dir.mkdir(parents=True,exist_ok=True);log_path=log_dir/(plan["attemptId"]+".log")
  ...
  if root and plan and consumed and consumed.exists():
   receipt=root/MANAGED/"receipts"/"post-apply-v2"/(plan["attemptId"]+".json")
   if not receipt.exists():
    message=PII.sub("[REDACTED_PII]",SECRET.sub("[REDACTED]",str(e)))[:1000]
    try:durable_json({"postApplyVerificationFailureReceiptV2Version":1,"attemptId":plan["attemptId"],"state":"EXECUTION_ERROR","errorType":type(e).__name__,"message":message,"approvalConsumed":True,"recordedAt":dt.datetime.now(dt.timezone.utc).isoformat()},receipt)
    except OSError:pass
