...
def load_context(root:Path,intake_path:Path)->tuple[dict,dict,Path]:
 intake=load_object(intake_path); validate_intake(intake,intake_path,root); intake_hash=sha(intake_path); receipt_path=root/".starter-harness/continuation-consumptions"/f"{intake_hash}.json"; receipt=load_object(receipt_path)
...
def head(root:Path,receipt:dict)->tuple[Path,dict]:
 ...
 while True:
  ...
  value=load_object(edge); required={"featureDraftUpdateVersion","previous","next","view","intakeSha256","state"}
  if set(value)!=required or value["featureDraftUpdateVersion"]!=1 or value["state"]!="COMMITTED" or value["previous"]!=current or value["intakeSha256"]!=intake_hash: raise ValueError("feature draft update evidence is invalid")
  view=value["view"]
