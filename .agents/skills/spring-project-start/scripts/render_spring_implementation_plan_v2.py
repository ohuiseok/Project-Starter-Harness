...
def render(plan,blockers):
 s=plan["summary"];ready=plan["status"]=="REVIEW_READY" and not blockers;capability_ready=plan["capability"]["planning"]=="SUPPORTED" and not any(i["source"]=="CAPABILITY_CATALOG" for i in plan["conflicts"]);dry=plan["advancement"]["codeDryRun"];lines=[f"# {esc(plan['featureId'])} 범용 Spring 구현 계획", "",f"- 상태: {plan['status']}",f"- 다음 단계 진행 가능: {'예' if ready else '아니요'}",f"- capability: {plan['capability']['adapterId']}",f"- API {s['operations']} · 컴포넌트 {s['components']} · 생성 {s['create']} · 확장 {s['extend']} · 재사용 {s['reuse']}",f"- 테스트 {s['tests']}개","","## 사용자 영향과 지원 범위","",f"- API-only: {'지원' if capability_ready else '현재 조합은 차단'}",f"- 보안: {'이번 작업에서 사용하지 않음' if plan['scope']['security']=='NOT_USED' else '현재 adapter 미지원'}","- DB·외부 client·build 변경: 이번 계획에서 사용하지 않음",f"- 코드 dry-run 준비: {'가능' if dry else '불가'} · {plan['advancement']['reason']}","","## API별 구현",""]
 ...
 lines.extend(["","## 현재 승인 효과","","- 이 단계 승인은 임시 후보 코드 준비만 허용","- 후보 검토, 격리 검증, 파일 적용은 각각 별도 승인 필요","- 소스, 테스트 실행, commit, push 변경 없음",""])
 return "\n".join(lines)
def main()->int:
