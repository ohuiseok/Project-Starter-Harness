...
def render(plan,blockers):
 ...
 for op in plan["operationLinks"]:lines.extend([f"- **{op['method']} `{esc(op['path'])}`** · `{esc(op['operationId'])}`",f"  - 구현 {len(op['componentRefs'])}개 · 테스트 {len(op['testRefs'])}개 · 요구사항 {len(op['requirementRefs'])}개",f"  - 보안 {'필요' if op['security']['required'] else '없음'} · 요구사항 {', '.join(esc(i) for i in op['requirementRefs']) or '없음'}"])
 ...
 for i in plan["components"]:lines.append(f"- {i['role']} · {i['disposition']} · `{esc(i['target']['path'])}` · {', '.join(sorted({s['action'] for s in i['symbols']}))}")
 lines.extend(["","## 선택",""])
