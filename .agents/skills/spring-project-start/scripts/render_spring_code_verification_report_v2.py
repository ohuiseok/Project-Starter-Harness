...
from spring_code_verification_v2 import render_report,validate_report
...
def main()->int:
 ...
 try:
  root=a.target.resolve(strict=True);report_path=a.report.resolve(strict=True);output=a.output.resolve();report=load_object(report_path);validate_report(report,report_path,root);expected=render_report(report)
  if a.check:
