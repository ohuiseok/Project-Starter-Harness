...
class ExecutionLoopV2Tests(unittest.TestCase):
 ...
 def run_attempt(self,root:Path,kind:str,attempt:str):
  ...
  argv=["run","--plan",str(plan_path),"--approval",str(approval),"--target",str(root),"--output",str(output)];sandbox=lambda workspace,home,command,approved=None:[str(workspace/command[0][2:]),*command[1:]]
