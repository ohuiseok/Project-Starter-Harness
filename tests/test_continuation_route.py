...
class ContinuationRouteTests(unittest.TestCase):
 ...
 def test_generic_next_uses_current_recommendation(self):
  with tempfile.TemporaryDirectory() as d:
   target,project,progress=self.fixture(Path(d)); route=self.route(target,project,progress,"다음 추천 기능 진행해줘"); self.assertEqual("NEXT_FEATURE",route["route"]["type"]); self.assertEqual("F002",route["route"]["selectedFeature"]["featureId"]); self.assertFalse(route["route"]["requiresConfirmation"])
