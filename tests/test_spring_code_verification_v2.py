...
class VerificationV2Tests(unittest.TestCase):
 ...
 def environment_value(self):return {"java":{"status":"READY","version":"Java 21"},"bubblewrap":{"status":"READY"},"runtimeMounts":{"state":"READY","mounts":[],"manifestSha256":"a"*64,"fileCount":0,"totalBytes":0}}
 ...
 def test_ready_plan_is_explicit_and_source_free(self):
  with tempfile.TemporaryDirectory() as d:
   ...
   with mock.patch.object(core,"validate_dry_run_approval",return_value=receipt),mock.patch.object(core,"validate_dry_run"),mock.patch.object(core,"cache",return_value=self.cache_value()),mock.patch.object(core,"git_state",return_value=self.git_value()),mock.patch.object(core,"environment",return_value=self.environment_value()):plan=build_plan(root,approval,90)
   self.assertTrue(plan["readyForApproval"]);self.assertEqual("DISABLED",plan["effects"]["network"]);self.assertFalse(plan["dependencyCache"]["credentialSettingsCopied"]);self.assertIn("Java 전용 runtime 설정",render_plan(plan));legacy=json.loads(json.dumps(plan));legacy["environment"].pop("runtimeMounts");self.assertNotIn("Java 전용 runtime 설정",render_plan(legacy));self.assertIn("비즈니스 행동 완료",render_plan(plan))
 def test_missing_offline_cache_is_visible_unknown_blocker(self):
  with tempfile.TemporaryDirectory() as d:
   root=Path(d);_,approval,receipt=self.fixture(root)
   with mock.patch.object(core,"validate_dry_run_approval",return_value=receipt),mock.patch.object(core,"validate_dry_run"),mock.patch.object(core,"cache",return_value=self.cache_value("MISSING")),mock.patch.object(core,"git_state",return_value=self.git_value()),mock.patch.object(core,"environment",return_value=self.environment_value()):plan=build_plan(root,approval)
 ...
 def test_secret_like_gradle_property_blocks_copy(self):
  with tempfile.TemporaryDirectory() as d:
   ...
   with mock.patch.object(core,"validate_dry_run_approval",return_value=receipt),mock.patch.object(core,"validate_dry_run"),mock.patch.object(core,"cache",return_value=self.cache_value()),mock.patch.object(core,"git_state",return_value=self.git_value()),mock.patch.object(core,"environment",return_value=self.environment_value()):plan=build_plan(root,approval)
 ...
 def test_runtime_mount_manifest_is_fixed_before_sandbox(self):
  with tempfile.TemporaryDirectory() as d:
   root=Path(d);workspace=root/"workspace";home=root/"home";workspace.mkdir();home.mkdir();approved=core.runtime_mounts();self.assertEqual("READY",approved["state"]);changed=dict(approved,manifestSha256="0"*64)
   with self.assertRaisesRegex(ValueError,"changed after approval"):runner.sandbox(workspace,home,["./gradlew","test"],changed)
